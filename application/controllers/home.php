	...
	*/
	class Home extends CI_CONTROLLER
	{
		...
		public function index(){
			...
			$query = $this->db->query('SELECT sum(jml_trans) as total_pembelian FROM `pembelian` WHERE `tgl_trans` between "2017-1-1" and "2017-12-31"')->row();
			$data['total_pembelian'] = $query->total_pembelian;
			$query = $this->db->query("SELECT * from barang")->result();
			$data['jumlah_barang'] = count($query);
			$query = $this->db->query("SELECT * from anggota")->result();
